...
@app.route("/api/goes")
def goes_proxy():
    """Proxy GOES-19 IR imagery from Iowa State Mesonet (open data, no key)
    so the frontend canvas can read pixels without CORS taint."""
    try:
        w = max(50, min(900, int(request.args.get("w", 230))))
        h = max(50, min(900, int(request.args.get("h", 215))))
        bbox = request.args.get("bbox", "")
        # basic validation: 4 comma-separated floats
        parts = [float(x) for x in bbox.split(",")]
        if len(parts) != 4:
            raise ValueError("bbox must be minlon,minlat,maxlon,maxlat")
        url = (
            "https://mesonet.agron.iastate.edu/cgi-bin/wms/goes_east.cgi"
            "?SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&LAYERS=conus_ch13&STYLES="
            "&SRS=EPSG:4326&BBOX=" + ",".join(f"{p:.4f}" for p in parts)
        ...
        r.headers["Cache-Control"] = "public, max-age=240"
        return r
    except Exception as exc:
